...
async def add_setting(
    ...
    if result is None:
        return await context.send_message(content=settings.message_add_setting_failed)
    return await context.send_message(
...
async def restrict_user(context: KoduckContext) -> discord.Message | None:
    ...
    # already restricted
    if user_level == 0:
        return await context.send_message(content=settings.message_restrict_failed)
...
async def unrestrict_user(context: KoduckContext) -> discord.Message | None:
    ...
    if user_level != 0:
        return await context.send_message(content=settings.message_unrestrict_failed)
